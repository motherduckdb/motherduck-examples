-- Create a scheduled Flight that runs this dbt project in MotherDuck compute.
--
-- Before running:
-- 1. Create the target database if needed:
--      CREATE DATABASE IF NOT EXISTS hacker_news_stats;
-- 2. Find an access token name:
--      SELECT * FROM md_access_tokens();
-- 3. Replace replace_with_your_token_name below.
-- 4. For scheduled workloads, replace REPO_REF with a tag or commit instead of main.
-- 5. For staging or non-production environments, set MOTHERDUCK_HOST in config.
