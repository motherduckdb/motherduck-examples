...
def sample_github_data():
    return [
        {"author": {"login": "user1"}, "weeks": [{"c": 5}, {"c": 3}, {"c": 2}]},
        {"author": {"login": "user2"}, "weeks": [{"c": 1}, {"c": 4}, {"c": 6}]},
    ]


def test_fetch_github_data():
    with patch("python_ingestion.load_to_motherduck_small.requests.get") as mock_get:
        mock_response = MagicMock()
        mock_response.json.return_value = [
            {"author": {"login": "user1"}, "weeks": [{"c": 5}]}
        ]
        ...
        mock_get.assert_called_once_with(
            "https://api.github.com/repos/duckdb/duckdb/stats/contributors"
        )
